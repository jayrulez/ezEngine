...
ezSharedPtr<Resource> VKSwapchain::GetBackBuffer(uint32_t buffer)
{
  return m_back_buffers[buffer];
}

uint32_t VKSwapchain::NextImage(const ezSharedPtr<Fence>& fence, uint64_t signal_value)
{
...
void VKSwapchain::Present(const ezSharedPtr<Fence>& fence, uint64_t wait_value)
{
  decltype(auto) vk_fence = fence->As<VKTimelineSemaphore>();
