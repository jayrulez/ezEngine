  ...
  , m_gpu_descriptor_pool(*this)
{
  ...
  for (auto& queue_info : m_queues_info)
  {
    ...
    m_command_queues[queue_info.first] = EZ_DEFAULT_NEW(VKCommandQueue, *this, queue_info.first, queue_info.second.queue_family_index);
  }
}

ezSharedPtr<Memory> VKDevice::AllocateMemory(uint64_t size, MemoryType memory_type, uint32_t memory_type_bits)
{
  return EZ_DEFAULT_NEW(VKMemory, *this, size, memory_type, memory_type_bits, nullptr);
}
...
ezSharedPtr<Swapchain> VKDevice::CreateSwapchain(Window window, uint32_t width, uint32_t height, uint32_t frame_count, bool vsync)
{
  return EZ_DEFAULT_NEW(VKSwapchain, *m_command_queues.at(CommandListType::kGraphics), window, width, height, frame_count, vsync);
}
...
ezSharedPtr<Fence> VKDevice::CreateFence(uint64_t initial_value)
{
  return EZ_DEFAULT_NEW(VKTimelineSemaphore, *this, initial_value);
}

ezSharedPtr<Resource> VKDevice::CreateTexture(TextureType type, uint32_t bind_flag, ezRHIResourceFormat::Enum format, uint32_t sample_count, int width, int height, int depth, int mip_levels)
{
...
ezSharedPtr<Resource> VKDevice::CreateBuffer(uint32_t bind_flag, uint32_t buffer_size)
{
  if (buffer_size == 0)
    return {};
...
ezSharedPtr<Shader> VKDevice::CreateShader(const ShaderDesc& desc, std::vector<uint8_t> byteCode, ezSharedPtr<ShaderReflection> reflection)
{
...
ezSharedPtr<Resource> VKDevice::CreateAccelerationStructure(AccelerationStructureType type, const ezSharedPtr<Resource>& resource, uint64_t offset)
{
  ezSharedPtr<VKResource> res = EZ_DEFAULT_NEW(VKResource, *this);
...
ezSharedPtr<QueryHeap> VKDevice::CreateQueryHeap(QueryHeapType type, uint32_t count)
{
