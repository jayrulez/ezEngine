...
DXGPUDescriptorPoolRange::DXGPUDescriptorPoolRange(
  ...
  //, m_callback(ezMakeScopeExit([m_offset = m_offset, m_size = m_size, m_pool = m_pool]() { m_pool.get().OnRangeDestroy(m_offset, m_size); }))
  //, m_callback(EZ_DEFAULT_NEW(ezDelegate<void(DXGPUDescriptorPoolRange*)>, [m_offset = m_offset, m_size = m_size, m_pool = m_pool](auto) { m_pool.get().OnRangeDestroy(m_offset, m_size); }))
  //, m_callback(EZ_DEFAULT_NEW(ezScopeExit<ScopeExitDelegate>, ezMakeScopeExit(ezMakeDelegate([m_offset = m_offset, m_size = m_size, m_pool = m_pool]() { m_pool.get().OnRangeDestroy(m_offset, m_size); }))))
{
  //ezUniquePtr<ezScopeExit<ScopeExitDelegate>> test;

  //ScopeExitDelegate delegate = [](DXGPUDescriptorPoolRange*) {};
}
