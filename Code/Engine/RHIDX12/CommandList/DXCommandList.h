...
private:
    ...
    void IASetVertexBufferImpl(uint32_t slot, const ezSharedPtr<Resource>& resource, uint32_t stride);
    void BuildAccelerationStructure(D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_INPUTS& inputs, const ezSharedPtr<Resource>& src, const ezSharedPtr<Resource>& dst, const ezSharedPtr<Resource>& scratch, uint64_t scratch_offset);
    ...
    std::map<uint32_t, ezSharedPtr<Resource>> m_lazy_vertex;
