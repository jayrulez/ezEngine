  ...
  , m_view_desc(m_view_desc)
{
  ...
  if (m_view_desc.bindless)
  {
    ...
    {
      m_range = EZ_DEFAULT_NEW(DXGPUDescriptorPoolRange, m_device.GetGPUDescriptorPool().Allocate(D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER, 1));
    }
    else
    {
      m_range = EZ_DEFAULT_NEW(DXGPUDescriptorPoolRange, m_device.GetGPUDescriptorPool().Allocate(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, 1));
    }
