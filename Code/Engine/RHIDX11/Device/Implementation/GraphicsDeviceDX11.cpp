...
void ezRHIGraphicsDeviceDX11::pso_validate(CommandList cmd)
{
  if (!dirty_pso[cmd])
    return;
  ...
  if (desc.bs != prev_bs[cmd] || desc.sampleMask != prev_samplemask[cmd] ||
      blendFactor[cmd].r != prev_blendfactor[cmd].r ||
      blendFactor[cmd].g != prev_blendfactor[cmd].g ||
      blendFactor[cmd].b != prev_blendfactor[cmd].b ||
      blendFactor[cmd].a != prev_blendfactor[cmd].a)
  {
    const float fact[4] = {blendFactor[cmd].r, blendFactor[cmd].g, blendFactor[cmd].b, blendFactor[cmd].a};
...
CommandList ezRHIGraphicsDeviceDX11::BeginCommandList(QUEUE_TYPE queue)
{
  CommandList cmd = cmd_count.PostIncrement();

  EZ_ASSERT_ALWAYS(cmd < COMMANDLIST_COUNT, "");
  ...
  stencilRef[cmd] = 0;
  blendFactor[cmd] = ezColor(1, 1, 1, 1);
...
void ezRHIGraphicsDeviceDX11::SubmitCommandLists()
{
  ...
  CommandList cmd_last = cmd_count.Set(0);
...
void ezRHIGraphicsDeviceDX11::commit_allocations(CommandList cmd)
{
...
void ezRHIGraphicsDeviceDX11::RenderPassBegin(const SwapChain* swapchain, CommandList cmd)
{
  swapchains[cmd].PushBack(swapchain);
  ...
  for (auto& attachment : desc.attachments)
  {
    ...
    else if (attachment.type == RenderPassAttachment::DEPTH_STENCIL)
    {
      ...
      if (attachment.loadop == RenderPassAttachment::LOADOP_CLEAR)
      {
        ...
        deviceContexts[cmd]->ClearDepthStencilView(DSV, _flags, texture->desc.clear.depthstencil.depth, (ezUInt8)texture->desc.clear.depthstencil.stencil);
      }
    }
  }
...
void ezRHIGraphicsDeviceDX11::RenderPassEnd(CommandList cmd)
{
...
void ezRHIGraphicsDeviceDX11::BindScissorRects(uint32_t numRects, const Rect* rects, CommandList cmd)
{
...
void ezRHIGraphicsDeviceDX11::BindViewports(uint32_t NumViewports, const Viewport* pViewports, CommandList cmd)
{
...
void ezRHIGraphicsDeviceDX11::BindResource(SHADERSTAGE stage, const GPUResource* resource, uint32_t slot, CommandList cmd, int subresource)
{
...
void ezRHIGraphicsDeviceDX11::BindResources(SHADERSTAGE stage, const GPUResource* const* resources, uint32_t slot, uint32_t count, CommandList cmd)
{
...
void ezRHIGraphicsDeviceDX11::BindUAV(SHADERSTAGE stage, const GPUResource* resource, uint32_t slot, CommandList cmd, int subresource)
{
  if (resource != nullptr && resource->IsValid())
  {
    auto internal_state = to_internal(resource);
...
void ezRHIGraphicsDeviceDX11::BindUAVs(SHADERSTAGE stage, const GPUResource* const* resources, uint32_t slot, uint32_t count, CommandList cmd)
{
...
void ezRHIGraphicsDeviceDX11::UnbindResources(uint32_t slot, uint32_t num, CommandList cmd)
{
...
void ezRHIGraphicsDeviceDX11::UnbindUAVs(uint32_t slot, uint32_t num, CommandList cmd)
{
  EZ_ASSERT_ALWAYS(num <= EZ_ARRAY_SIZE(__nullBlob), "Extend nullBlob to support more resource unbinding!");
...
void ezRHIGraphicsDeviceDX11::BindSampler(SHADERSTAGE stage, const Sampler* sampler, uint32_t slot, CommandList cmd)
{
...
void ezRHIGraphicsDeviceDX11::BindConstantBuffer(SHADERSTAGE stage, const GPUBuffer* buffer, uint32_t slot, CommandList cmd)
{
...
void ezRHIGraphicsDeviceDX11::BindVertexBuffers(const GPUBuffer* const* vertexBuffers, uint32_t slot, uint32_t count, const uint32_t* strides, const uint32_t* offsets, CommandList cmd)
{
...
void ezRHIGraphicsDeviceDX11::BindIndexBuffer(const GPUBuffer* indexBuffer, const INDEXBUFFER_FORMAT format, uint32_t offset, CommandList cmd)
{
...
void ezRHIGraphicsDeviceDX11::BindStencilRef(uint32_t value, CommandList cmd)
{
  stencilRef[cmd] = value;
}
void ezRHIGraphicsDeviceDX11::BindBlendFactor(float r, float g, float b, float a, CommandList cmd)
{
  blendFactor[cmd].r = r;
  blendFactor[cmd].g = g;
  blendFactor[cmd].b = b;
...
void ezRHIGraphicsDeviceDX11::BindPipelineState(const PipelineState* pso, CommandList cmd)
{
...
void ezRHIGraphicsDeviceDX11::BindComputeShader(const Shader* cs, CommandList cmd)
{
...
void ezRHIGraphicsDeviceDX11::Draw(uint32_t vertexCount, uint32_t startVertexLocation, CommandList cmd)
{
  pso_validate(cmd);
...
void ezRHIGraphicsDeviceDX11::DrawIndexed(uint32_t indexCount, uint32_t startIndexLocation, uint32_t baseVertexLocation, CommandList cmd)
{
  pso_validate(cmd);
  commit_allocations(cmd);
...
void ezRHIGraphicsDeviceDX11::DrawInstanced(uint32_t vertexCount, uint32_t instanceCount, uint32_t startVertexLocation, uint32_t startInstanceLocation, CommandList cmd)
{
  pso_validate(cmd);
  commit_allocations(cmd);
...
void ezRHIGraphicsDeviceDX11::DrawIndexedInstanced(uint32_t indexCount, uint32_t instanceCount, uint32_t startIndexLocation, uint32_t baseVertexLocation, uint32_t startInstanceLocation, CommandList cmd)
{
  pso_validate(cmd);
  commit_allocations(cmd);
...
void ezRHIGraphicsDeviceDX11::DrawInstancedIndirect(const GPUBuffer* args, uint32_t args_offset, CommandList cmd)
{
  pso_validate(cmd);
  commit_allocations(cmd);
...
void ezRHIGraphicsDeviceDX11::DrawIndexedInstancedIndirect(const GPUBuffer* args, uint32_t args_offset, CommandList cmd)
{
  pso_validate(cmd);
  commit_allocations(cmd);
...
void ezRHIGraphicsDeviceDX11::Dispatch(uint32_t threadGroupCountX, uint32_t threadGroupCountY, uint32_t threadGroupCountZ, CommandList cmd)
{
  commit_allocations(cmd);

  deviceContexts[cmd]->Dispatch(threadGroupCountX, threadGroupCountY, threadGroupCountZ);
}
void ezRHIGraphicsDeviceDX11::DispatchIndirect(const GPUBuffer* args, uint32_t args_offset, CommandList cmd)
{
  commit_allocations(cmd);

  deviceContexts[cmd]->DispatchIndirect((ID3D11Buffer*)to_internal(args)->resource.Get(), args_offset);
}
void ezRHIGraphicsDeviceDX11::CopyResource(const GPUResource* pDst, const GPUResource* pSrc, CommandList cmd)
{
...
void ezRHIGraphicsDeviceDX11::UpdateBuffer(const GPUBuffer* buffer, const void* data, CommandList cmd, int dataSize)
{
...
void ezRHIGraphicsDeviceDX11::QueryBegin(const GPUQueryHeap* heap, uint32_t index, CommandList cmd)
{
  auto internal_state = to_internal(heap);
  deviceContexts[cmd]->Begin(internal_state->resources[index].Get());
}
void ezRHIGraphicsDeviceDX11::QueryEnd(const GPUQueryHeap* heap, uint32_t index, CommandList cmd)
{
  auto internal_state = to_internal(heap);
  deviceContexts[cmd]->End(internal_state->resources[index].Get());
}

ezRHIGraphicsDevice::GPUAllocation ezRHIGraphicsDeviceDX11::AllocateGPU(size_t dataSize, CommandList cmd)
{
...
void ezRHIGraphicsDeviceDX11::EventBegin(const char* name, CommandList cmd)
{
...
void ezRHIGraphicsDeviceDX11::EventEnd(CommandList cmd)
{
  userDefinedAnnotations[cmd]->EndEvent();
}
void ezRHIGraphicsDeviceDX11::SetMarker(const char* name, CommandList cmd)
{
  ezStringWChar text(name);
  if (text.GetElementCount() > 0)
  {
